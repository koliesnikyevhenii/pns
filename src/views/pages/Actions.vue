...
<template>
  <div class="card">
    ...
    <div class="flex mb-4">
      <Button type="button" icon="pi pi-file-plus" label="Add Action" outlined @click="showActionDialog" />
    </div>

    <DataTable stripedRows sortable :value="actionItems" :paginator="true" :rows="10" dataKey="id" :rowHover="true"
      v-model:filters="filters" :loading="loading" :filters="filters" :globalFilterFields="['action', 'description']"
      showGridlines :rowsPerPageOptions="[5, 10, 25]"
      ...
      <Column header="Description" field="description" style="min-width: 12rem" :sortable="true">
        <template #body="{ data }">
          {{ data.description }}
        </template>
      </Column>
      <Column field="NumberOfMessageSent" style="min-width: 12rem">
        <template #body="{ data }">
          <Button icon="pi pi-trash" outlined rounded severity="danger" @click="confirmDeleteAction(data)" />
        </template>
      </Column>
    </DataTable>
  </div>

  <Dialog v-model:visible="deleteActionDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
    <div class="flex items-center gap-4">
      <i class="pi pi-exclamation-triangle !text-3xl" />
      <span v-if="action">Are you sure you want to delete <b>{{ action.action }}</b>?</span>
    </div>
    <template #footer>
  ...
  <Dialog v-model:visible="createActionDialog" :style="{ width: '450px' }" header="Create Action" :modal="true">
    <Form @submit="submitForm">
      <div class="flex items-center gap-4">
        <div class="flex flex-col gap-2">
          <label for="actionName">Action Name</label>
          <InputText id="actionName" type="text" v-model="actionName" v-bind="actionNameAttr" />
